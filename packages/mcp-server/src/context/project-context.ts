...
export class AIContextProvider {
   ...
   */
  async getTaskContext(taskDescription: string): Promise<{
  ...
  private async discoverPackages(): Promise<ProjectContext['packages']> {
  ...
  private inferPackageDescription(name: string): string {
  ...
  private async discoverSchemas(): Promise<ProjectContext['schemas']> {
  ...
  private extractSchemaFields(content: string): string[] {
    const fields: string[] = [];
  ...
  private async loadKnownPitfalls(): Promise<string[]> {
    const aiIndexPath = path.join(this.projectRoot, '.japavel', 'AI_INDEX.md');
    if (!fs.existsSync(aiIndexPath)) return [];
  ...
  private async findRelevantSchemas(keywords: string[]): Promise<string[]> {
    const schemasDir = path.join(this.projectRoot, 'packages', 'contracts', 'src', 'schemas');
    if (!fs.existsSync(schemasDir)) return [];
  ...
  private async findRelevantPatterns(keywords: string[]): Promise<string[]> {
    const aiIndexPath = path.join(this.projectRoot, '.japavel', 'AI_INDEX.md');
    if (!fs.existsSync(aiIndexPath)) return [];
  ...
  private async findRelevantPitfalls(keywords: string[]): Promise<string[]> {
    return this.loadKnownPitfalls();
  }

  private suggestFix(errorType: string, message: string): string {
