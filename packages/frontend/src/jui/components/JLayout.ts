...
export default defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const navLinkClass = (isActive: boolean) => css({
      fontWeight: '600',
      color: isActive ? theme.colors.brand.primary : theme.colors.neutral.gray500,
      fontSize: '0.9375rem',
      textDecoration: 'none',
    ...
    const sidebarItem = (path: string) => {
      ...
      return css({
        ...
        fontWeight: '600',
        color: isActive ? theme.colors.brand.primary : theme.colors.neutral.gray600,
        fontSize: '1rem',
        textDecoration: 'none',
        display: 'flex',
        alignItems: 'center',
      ...
      return h('div', { class: layoutStyle }, [
        ...
        h('div', { class: headerContainer }, [
          h('header', { class: headerStyle }, [
            h('div', { class: headerContent }, [
              h(RouterLink, { to: '/', class: css({ textDecoration: 'none' }) }, () => 
                h('div', { class: logoStyle }, 'Japavel')
              ),
              h('nav', { class: navStyle }, [
                h(RouterLink, { to: '/dashboard', class: navLinkClass(route.path === '/dashboard') }, () => 'Dashboard'),
                h(RouterLink, { to: '/settings', class: navLinkClass(route.path === '/settings') }, () => 'Settings'),
                h(RouterLink, { to: '/login', class: navLinkClass(false) }, () => 'Logout'),
              ])
            ])
          ]),
        ]),
        ...
        h('div', { class: mainContainer }, [
          h('div', { class: mainInner }, [
            props.withSidebar ? h('aside', { class: sidebarStyle }, [
              h(RouterLink, { to: '/dashboard', class: sidebarItem('/dashboard') }, () => 'Overview'),
              h(RouterLink, { to: '/analytics', class: sidebarItem('/analytics') }, () => 'Analytics'),
              h(RouterLink, { to: '/users', class: sidebarItem('/users') }, () => 'Users'),
            ]) : null,
