...
export class VerificationLoop {
     ...
     * TypeScript check
     */
    async runTypeScriptCheck(startTime) {
        try {
     ...
     * ESLint check
     */
    async runEslintCheck(startTime) {
        try {
     ...
     * Prettier check
     */
    async runPrettierCheck(startTime) {
        try {
     ...
     * Prisma check
     */
    async runPrismaCheck(startTime) {
        try {
     ...
     */
    async runAtomicRulesCheck(startTime) {
        const errors = [];
        const srcDirs = [
        ...
        return this.createResult('atomic-rules', errors.length === 0, startTime, errors.length === 0 ? 'All atomic rules pass' : `Found ${errors.length} violations`, errors);
    }
    /**
     * Security check
     * Basic security pattern checks
     */
    async runSecurityCheck(startTime) {
        const errors = [];
        const srcDirs = [
        ...
        return this.createResult('security', errors.length === 0, startTime, errors.length === 0 ? 'No security issues found' : `Found ${errors.length} potential issues`, errors);
    }
    /**
     ...
     * Create check result
     */
    createResult(name, passed, startTime, output, errors) {
        return {
            name,
            passed,
 ...
 */
export const createVerificationLoop = (projectRoot, options) => {
    return new VerificationLoop({ projectRoot, ...options });
};
/**
 * Run verification and return result
 */
export const runVerification = async (projectRoot, options) => {
 ...
 */
export const quickVerify = async (projectRoot) => {
 ...
 */
export const fullVerify = async (projectRoot) => {
