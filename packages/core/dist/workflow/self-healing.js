...
export class SelfHealingWorkflow {
     ...
     */
    async fixTypeScriptError(error, filePath) {
        ...
        switch (error.code) {
            case 'TS7006': // Parameter implicitly has any type
                ...
                if (paramMatch) {
                    lines[lineIndex] = lines[lineIndex].replace(new RegExp(`(${paramMatch[1]})\\s*([,)])`), '$1: unknown$2');
            ...
            case 'TS2531': // Object is possibly null
                ...
                if (propMatch) {
                    lines[lineIndex] = lines[lineIndex].replace(new RegExp(`\\.${propMatch[1]}`), `?.${propMatch[1]}`);
     ...
     * Fix ESLint error
     */
    async fixEslintError(error, filePath) {
        ...
        switch (error.code) {
            case '@typescript-eslint/no-unused-vars':
                ...
                if (varMatch && !varMatch[1].startsWith('_')) {
                    lines[lineIndex] = lines[lineIndex].replace(new RegExp(`\\b${varMatch[1]}\\b`), `_${varMatch[1]}`);
     ...
     */
    createResult(success, allErrors, allFixes, attempts, remainingErrors) {
 ...
 */
export const createSelfHealingWorkflow = (projectRoot, options) => {
    return new SelfHealingWorkflow({ projectRoot, ...options });
};
/**
 * Run self-healing workflow and return result
 */
export const runSelfHealing = async (projectRoot, options) => {
