import { getNormalizedFields } from './parser';
/**
 * Generate all code artifacts from a JapavelSchema
 */
export const generateAll = (schema) => {
    return {
 ...
 */
export const generateTrpcRouter = (schema) => {
    const model = schema.Model;
...
function getProcedureType(auth) {
    return auth === 'public' ? 'publicProcedure' : 'protectedProcedure';
}
function generateCreateProcedure(model, modelLower, config) {
...
function generateReadProcedure(model, modelLower, config) {
    const procedure = getProcedureType(config.auth);
    return `
  getById: ${procedure}
...
function generateUpdateProcedure(model, modelLower, config) {
    const procedure = getProcedureType(config.auth);
    return `
  update: ${procedure}
...
function generateDeleteProcedure(model, modelLower, config) {
    const procedure = getProcedureType(config.auth);
    return `
  delete: ${procedure}
    .input(z.object({ id: z.string().uuid() }))
...
function generateListProcedure(model, modelLower, config) {
    const procedure = getProcedureType(config.auth);
    return `
  list: ${procedure}
...
function generateSearchProcedure(model, modelLower, config) {
    const procedure = getProcedureType(config.auth);
    return `
  search: ${procedure}
    .input(z.object({
 ...
 */
export const generatePrismaModel = (schema) => {
    const fields = getNormalizedFields(schema);
    const model = schema.Model;
    const fieldLines = Object.entries(fields)
        .map(([name, field]) => {
...
export const generateReactComponent = (schema) => {
    ...
    const displayFields = viewConfig.fields || Object.keys(fields).filter(f => !['id', 'createdAt', 'updatedAt'].includes(f));
...
function generateFormField(name, field) {
 ...
 */
export const generateTypes = (schema) => {
    const model = schema.Model;
    const fields = getNormalizedFields(schema);
