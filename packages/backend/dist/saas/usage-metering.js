...
export class UsageMeteringService {
    metrics = new Map();
    limits = new Map(); // tenantId -> metric -> limit
    records = [];
    aggregates = new Map(); // key -> value
    /**
     ...
     */
    setLimitsFromPlan(tenantId, planLimits) {
        for (const [metric, limit] of Object.entries(planLimits)) {
            if (limit < 0)
                continue; // -1 = unlimited
    ...
    async record(record) {
        ...
        for (const period of metric.periods) {
            const key = this.getAggregateKey(record.tenantId, record.metric, period, record.timestamp);
            switch (metric.type) {
                case 'count':
     ...
     */
    async recordForCurrentTenant(metricName, value, dimensions) {
     ...
     */
    async increment(tenantId, metric, amount = 1) {
        await this.record({
            tenantId,
            metric,
            value: amount,
     ...
     */
    getUsage(tenantId, metric, period = 'month') {
     ...
     */
    getUsageSummary(tenantId, metric, period = 'month') {
     ...
     */
    async checkLimit(tenantId, metric, additionalUsage = 0) {
        const limit = this.limits.get(tenantId)?.get(metric);
        if (!limit) {
     ...
     */
    async enforceLimit(tenantId, metric, additionalUsage = 0) {
        const result = await this.checkLimit(tenantId, metric, additionalUsage);
        if (!result.allowed) {
            throw new UsageLimitExceededError(result.message || `Usage limit exceeded for ${metric}`, metric, result.current, result.limit);
        }
    }
    /**
     * Reset usage for a period (e.g., at billing cycle)
     */
    resetUsage(tenantId, metric, period) {
        const key = this.getAggregateKey(tenantId, metric, period, new Date());
     ...
     * Get usage history
     */
    getHistory(tenantId, metric, options) {
        let records = this.records.filter(r => r.tenantId === tenantId && r.metric === metric);
...
export class UsageLimitExceededError extends Error {
    ...
    current;
    limit;
    constructor(message, metric, current, limit) {
        super(message);
        this.metric = metric;
 ...
 */
export const createUsageMeteringService = () => {
