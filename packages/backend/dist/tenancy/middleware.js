...
import { runWithTenantAsync, createTenantContext, hasRole, hasPermission, } from './context';
const defaultConfig = {
    strategy: 'header',
 ...
 */
export const resolveTenantId = async (req, config) => {
 ...
 */
export const createTenantMiddleware = (getTenant, getMember, config = {}) => {
 ...
 */
export const requireRoleMiddleware = (role) => {
    return async (opts) => {
 ...
 */
export const requirePermissionMiddleware = (permission) => {
    return async (opts) => {
        const ctx = opts.ctx.tenantContext;
        if (!ctx) {
            throw new TRPCError({
